        ...
        for target in targets:
            for tarLabel in range(target.shape[1]):
                ...
                for model in models:
                    ...
                    #    break
                    phase = 1
                    for sample in range(model[0].shape[0]):
                        cbks = []
                        Wsub = []
                        params = []
                        source_estimators = []
                        ...
                        for source in model:
                            phase = 1
                            # Split dataset
                            labels = np.asarray(range(source.shape[0]))
                            labels = np.delete(labels, sample)

                            xtrain = source[labels,:]
                            xtest = source[sample,:]
                        ...
                        if model_counter == 0:
                            stats1.append(data[0:6])
                            params1.append(data[6:9])
                            w1.append(estimator.smkl.get_kernel_weights())

                        elif model_counter == 1:
                            stats2.append(data[0:6])
                            params2.append(data[6:9])
                            w2.append(estimator.smkl.get_kernel_weights())

                        else:
                            stats3.append(data[0:6])
                            params3.append(data[6:9])
