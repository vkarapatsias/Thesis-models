...
class SMKL(BaseEstimator):
    ...
    def __init__(self, Omega = 1, kernel = rbf_kernel, similarity = 'KA', kernel_params = None, baseKernels = None):
        """
        Parameters
        ----------
        Omega : float or array-like, shape = (n_kernels, )
        ...
        if(self.BaseKernels) is not None:
            self.nKernels = np.asarray(self.BaseKernels).shape[0]
    ...
    def make_combined_kernel(self, X, Z = None, phase = 1, previousWeights = None):
        """
        Uses optimized kernel weights to compute kernel combination.
            ...
            if Z is None:
                ...
                if phase == 1:
                    ...
                    for i in range(self.kernel_params.shape[0]): #for each feature
                        ...
                        temp = np.dstack(temp).dot(previousWeights[i]) #multiply with the weights in each feature
                        self.BaseKernels.append(temp)
                elif phase == 3:
                    # 13 RBF x 39 times => 39 cbks (30x30) => 2 scbks => final combined kernel
                    Wsource = previousWeights[0] #weights for the scbks' combination
                    Wcbks = previousWeights[1] #weights for the cbks' combination
                    Scbks_kernels = [] #sources's combined kernels
                    for ii in range(np.asarray(Wsource).shape[0]): #for every source
                        x = X[ii]
                        z = Z[ii]
                        self.BaseKernels = []
                        for i in range(x.shape[1]): #for each feature
                            temp = [self.kernel(x[:,i].reshape(-1,1), z[:,i].reshape(-1,1), param) for param in self.kernel_params[ii][i]] #for each gamma parameter
                            temp = np.dstack(temp).dot(Wcbks[ii][i]) #multiply with the weights in each feature
                            self.BaseKernels.append(temp)

                        temp = [self.kernel(x, z, param) for param in self.kernel_params[ii][i]] #across features
                        temp = np.dstack(temp).dot(Wcbks[ii][i+1]) #multiply with the weights in each feature
                        self.BaseKernels.append(temp)

                        temp = np.dstack(self.BaseKernels).dot(Wsource[ii])
                        Scbks_kernels.append(temp)
                    self.BaseKernels = Scbks_kernels
    ...
    def HSIC(self, X = None, y = None, mode = 'combo'):
        """
        Hilbert-Schmidt Independence Criterion Calculation.
...
class SMKL_SVR(BaseEstimator):
    ...
    def __init__(self, C = 1, epsilon = 1e-1, kernel = rbf_kernel, Omega = 1, similarity = 'KA', kernel_params = None, bKernels = None, phase = 1, bweights = None):
        """
        Parameters
        ----------
        C : float
        ...
        if phase == 1:
            ...
            # self.BaseKernels[0]).shape = (numOfSources,30,30)
            self.smkl = SMKL(Omega = self.Omega, kernel = self.kernel, similarity = self.similarity, kernel_params = self.kernel_params, baseKernels = self.BaseKernels[0])
    ...
    def fit(self, X, y):
        ...
        self._xtrain = X
        self.smkl.fit(X, y, self.ph)
        self.smkl.make_combined_kernel(X,phase = self.ph)
    ...
    def predict(self, X):
        ...
        self._xtest = X
        self.smkl.make_combined_kernel(self._xtrain, self._xtest, self.ph, self.prevWeights)
        ckernel = self.smkl.get_combined_kernel()
    ...
    def stats(self,X,y):
        ...
        CVrmse = 100*rmse/np.mean(y)
        mae = mean_absolute_error(y,ypred)
        return rmse, CVrmse, mae, ypred
