...
path_list = ["results/ss/input/fixed/","results/ss/input/adaptive/"]
    ...
    for j in range(1,4):
        ...
        for jj in range(1,upper):
            for k in range(outputs):
                ...
                _r2.append(r2_score(y, ypred))
                _rmse.append(math.sqrt(mean_squared_error(y,ypred)))
                temp= 100*(math.sqrt(mean_squared_error(y,ypred)))/np.mean(y)
                _CVrmse.append(temp)
                _mae.append(mean_absolute_error(y,ypred))
...
with open('evaluation/r2.csv', 'w+') as csvFile:
    ...
    writer.writerow(['scenario1','scenario2'])
